...
`design-system-creator`, `using-git-worktrees`, `verify-build`, `verify-diff-claims` permanecem instaladas e dispará­veis (frontmatter `name`/`description` preservado em conteúdo equivalente). **Cláusula aditiva (v1.1):** o adapter gerado pode conter skills NOVAS do Forge (contagem `>= 4` no modo generated; primeira adição: `gate-runner`, W2.2). O snapshot (source) permanece exatamente 4, e as 4 skills legadas são exigidas em ambos os modos. **Escopo do frontmatter (v1.4):** a exigência de frontmatter YAML com `description` recai sobre o `SKILL.md` de cada skill, que é o ponto de entrada carregado pelo agente. Uma skill pode trazer `references/*.md`, `scripts/` e `assets/` — material lido sob demanda, que é o mecanismo de progressive disclosure e por definição não entra no contexto junto com a descrição. Esses recursos não carregam frontmatter e não são validados como skills.

### C5 — Hooks (5 scripts; 1 wired na baseline, +2 opt-in)
...
**Estado de referência fiel (baseline):** o `settings.json` atual wira **apenas** `enforce-worktree-location.sh` (PreToolUse, matcher `Bash`, via `$CLAUDE_PROJECT_DIR`). O adapter gerado preserva exatamente esse wiring (apontando para o novo path em `.forge/hooks/`), sem ativar os demais hooks silenciosamente — qualquer ativação adicional é mudança de comportamento e exige decisão explícita, fora deste contrato.

**Estado opt-in (+2 Session hooks — v1.5, LDG-0022):** quando `handoff.auto: true` no `forge.yaml`, o adapter gerado wira `SessionStart` (`hooks/session/on-session-start.sh`) e `SessionEnd` (`hooks/session/on-session-end.sh`) além do worktree-guard — 3 entradas `"command":` no total. A árvore gerada NASCE com `auto: false` (por isso o teste da baseline acima continua exigindo exatamente 1 wiring), e o estado opt-in é uma cláusula ADITIVA: o `claude-contract.bats` afirma os DOIS estados — baseline diretamente contra `$CLAUDE_CONTRACT_TARGET`, e o opt-in construindo, dentro do próprio teste (modo generated), uma cópia privada de `$CLAUDE_CONTRACT_TARGET` com `handoff.auto: true` e um re-sync do adapter claude. Antes da v1.5 esta cobertura vivia só em `tests/w62-handoff-hook-gate.sh` — funcionalmente equivalente, mas fora do C5 literal; a v1.5 consolida sem remover o w62 (ele continua cobrindo o fluxo de `/forge:init` ponta a ponta).
