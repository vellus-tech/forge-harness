...
|---|---|---|
| `pre-commit` | antes do commit | gates rápidos: parseabilidade, Unicode (anti em-dash), anti-debug, lint dos arquivos staged. Output de 1 linha. Roda também o `check-secrets.sh staged` (rule `conventions/no-hardcoded-secrets.md`): segredo hardcoded é barato de barrar no índice e caro depois, porque o commit distribui o valor para todos os clones e removê-lo vira rotação de credencial. |
| `check-secrets` (gate) | `pre-commit` (`staged`), CI sobre o diff do PR (`range`), `runtime.gates` do `pre-push` (`--path`), e `report` sob demanda | reprova credencial de conexão, chave privada PEM, token de provedor e `Authorization: Basic` literal em arquivo **versionado** (`git ls-files`), comentário incluído. Duas propriedades inegociáveis: **nunca passa por vacuidade** — conjunto varrido vazio (path errado, glob que não casa, allowlist ampla demais) é `FAIL` explícito e jamais `OK`, porque "não procurei" não pode virar "não encontrei"; e **falha de integridade ignora o modo** — `secrets.enforce: warn` no `forge.yaml` rebaixa achado (adoção incremental em brownfield com passivo), nunca conjunto vazio nem allowlist malformada. Exceções em `.forge/secrets-allowlist.txt` exigem justificativa na própria linha, e a falta dela reprova o gate em vez de virar passe livre. O modo `report` inventaria o passivo sem reprovar, para o brownfield medir antes de escolher a política. |
| `pre-push` | antes do push da branch de trabalho | typecheck + testes unitários + gates import-resolve/consistency. Bloqueia push se falhar. Re-exporta o stdin do git (`<local ref> <local sha> <remote ref> <remote sha>` por linha) num arquivo temporário apontado por **`FORGE_PUSH_REFS_FILE`**, para que um gate de `runtime.gates` saiba **o que está sendo publicado** — sem isso, um gate que resolve o alvo por `git rev-parse HEAD` valida o que está em checkout, que diverge do que o push publica em `git push origin outra-branch:main` e em push de múltiplas refs. Por arquivo e não com o payload dentro da variável: o kernel limita uma string de ambiente a 128 KB no Linux e o argv+envp a ~1 MB no macOS, e acima disso todo `exec` do hook falha com E2BIG — inclusive o `awk` que lê este próprio FORGE.md, fazendo o hook anunciar `pre-push OK` sem ter rodado gate nenhum. Push sem refs escreve arquivo **vazio** (distinguível de uma linha em branco); o arquivo é removido no `EXIT` do hook. |
