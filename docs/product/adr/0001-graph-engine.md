...
- **2026-07-16 (gatilho v0.2 avaliado — issue #18):** o piloto `axis-fare-validator` (app Android Java, 259 `.java`) expôs **ausência total de cobertura** para Java — grafo com 2 nodes (blobs de firmware), 0 edges, e cascata vazia em c4/onboard/impact. O caso é mais forte que o gatilho registrado ("perda de precisão relevante"), mas **não** exigiu tree-sitter: como Java segue a convenção pacote=diretório, um extractor heurístico zero-dep (regex de `import` + índice FQN→arquivo, no mesmo molde de C#/JS) resolve edges internos de forma determinista. Kotlin (que já tinha nodes mas 0 edges) ganhou os edges no mesmo extractor. tree-sitter (Opção 3) **permanece adiado** — o gatilho ainda não disparou para uma linguagem cuja resolução import→arquivo dependa de AST. Além do extractor, o `forge validate graph` passou a **falhar/avisar** quando a linguagem dominante do repo tem 0 nodes (o "OK" anterior sobre 2 nodes era enganoso), e o `impact-scan` distingue "seed fora do grafo" de "linguagem do seed não coberta pelo extractor".
