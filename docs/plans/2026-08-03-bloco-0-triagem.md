...
- **Identidade única**: conforme, sem trabalho. As 8 ocorrências de `MSA`/`MSP` no repositório inteiro estão todas dentro do próprio store do canal (`CHANNEL.md` e os `.jsonl`) — zero em código. `ConnectionProfile.TenantId` é o único identificador de conta, e é o que compõe headers/paths multi-tenant, a chave de idempotência de tap (`IdempotencyKeyDeriver`) e o tópico de telemetria `device/{tenantId}/telemetry/v1`.
...
>
> Precisamos da confirmação formal de três pontos do schema do `TransactionResponse` JSON: qual campo corresponde ao hash do PAN e com que algoritmo e comprimento ele é gerado (observamos 16 ou 32 bytes em hex, e precisamos saber o que determina cada caso); se e como o PAR EMV (tag `0x9F24`) é exposto; e a lista completa dos campos que a resposta pode carregar, incluindo os que só aparecem sob configuração de projeto — identificamos `last4`, `pan_index`, `card_bin` e `uid` nessa condição, e precisamos saber se há outros e o que ativa cada um.
>
