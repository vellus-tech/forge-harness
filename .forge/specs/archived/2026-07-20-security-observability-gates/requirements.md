...
- **Quando** o harness é instalado ou atualizado, **o sistema deve** carregar uma constitution que declara o princípio universal de segurança (no secrets, least privilege, auditability, security by default) e, para os projetos que ativam os rule-packs opt-in `authz` e `pii-pci`, uma remissão explícita às cláusulas específicas dessas packs — (a) toda decisão de acesso passa por um Policy Decision Point (PDP); (b) deny-by-default e fail-closed; (d) nenhum PII/PAN em log — que passam a valer como contrato obrigatório na constitution local do projeto, não como invariante universal do harness.
- **Critérios de aceite:**
  - [ ] `template/.forge/constitution.md`, item "Security by default", mantém apenas o princípio universal e substitui as cláusulas específicas por uma frase remetendo aos packs `authz`/`pii-pci` (`rules/architecture/authz-pdp-pep.md`, `rules/architecture/pii-pci-classification.md`).
  - [ ] `validate-rules.sh` não acusa drift após a emenda.
  - [ ] As cláusulas PDP/deny-by-default/fail-closed permanecem no corpo de `authz-pdp-pep.md` (não na constitution), e a rule carrega `pack: authz` + `opt_in: true` no frontmatter; `pii-pci-classification.md` carrega `pack: pii-pci` + `opt_in: true`.
- **Rastreia:** proposal §2 (Constitution), §4 (risco falso-negativo).
- **Decisão revisada (2026-07-20, pré-merge):** a versão original deste requisito colocava as cinco cláusulas como invariantes **universais** da constitution, aplicáveis a todo projeto que instala o harness. Decisão do Milton: tornar **opt-in por família** — nem todo projeto (ex.: `axis-fare-validator`) deve herdar a obrigatoriedade de PDP/deny-by-default/PII-log que faz sentido para outros (ex.: `axis-go-cloud`). A constitution volta a ser genérica; as cláusulas específicas migram para o corpo das rules `authz-pdp-pep.md`/`pii-pci-classification.md`, marcadas `pack:`/`opt_in: true`, e só valem como contrato nos projetos que ativam o pack. A maquinaria de ativação (installer materializando só packs ativos) fica para um change futuro.
- **Decisão de escopo — audit trail:** das quatro famílias (proposal §1), *audit trail* é imposta de forma **declarativa**, não por gate de varredura de código. Motivo: "toda mutação emite audit event" não é detectável estaticamente de forma genérica sem falso-positivo alto (exigiria saber quais funções são mutações). A imposição de audit trail é a tríade: (i) o princípio universal de auditabilidade na constitution (item 7); (ii) o **mapa de eventos auditáveis** obrigatório no template (REQ-13), conferido pelo `validate-spec`; (iii) a rule `domain/audit-immutability.md` existente, referenciada por `pii-pci-classification.md` (REQ-03). A execução/emissão real do evento é do projeto consumidor (fora de escopo).
