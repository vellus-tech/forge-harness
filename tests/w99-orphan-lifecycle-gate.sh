...
b2="$(buckets 2>&1 || true)"
grep -qE "^merged_unarchived"$'\t'"chg-done"$'\t' <<<"$b2" && { echo "FAIL: chg-done duplicado em merged_unarchived"; exit 1; }
echo "OK [2]"
...
b3="$(buckets 2>&1 || true)"
grep -qE $'\t'"chg-impl"$'\t' <<<"$b3" && { echo "FAIL: chg-impl (implemented, sem branch) não deveria ser órfão"; buckets; exit 1; }
...
# idempotente
av_noop="$(FORGE_ROOT="$T" bash "$AV" paymod implementing 2>&1 || true)"
grep -q NOOP <<<"$av_noop" || { echo "FAIL: 2ª chamada implementing não foi NOOP ($av_noop)"; exit 1; }
...
av_skip="$(FORGE_ROOT="$T" bash "$AV" modulo-fantasma implemented 2>&1 || true)"
grep -q SKIP <<<"$av_skip" || { echo "FAIL: módulo sem change não deu SKIP ($av_skip)"; exit 1; }
echo "OK [6]"
...
b8="$(buckets 2>&1 || true)"
grep -qE $'\t'"chg-auth"$'\t' <<<"$b8" && { echo "FAIL: 'chg-auth' virou órfão por substring de 'oauth2'"; buckets; exit 1; }
echo "OK [8]"
