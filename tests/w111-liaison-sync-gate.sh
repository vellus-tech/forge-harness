...
tl3="$(LG fv thread list "$CH" 2>&1)"; tl3_rc=$?
[ "$tl3_rc" -eq 0 ] || { echo "FAIL [3]: thread list reprovou (rc $tl3_rc): $tl3"; exit 1; }
...
tl9a="$(LG gc thread list "$CH" 2>&1)"; tl9a_rc=$?
[ "$tl9a_rc" -eq 0 ] || { echo "FAIL [9a]: thread list reprovou (rc $tl9a_rc): $tl9a"; exit 1; }
...
tl9b="$(LG gc thread list "$CH" 2>&1)"; tl9b_rc=$?
[ "$tl9b_rc" -eq 0 ] || { echo "FAIL [9b]: thread list reprovou (rc $tl9b_rc): $tl9b"; exit 1; }
...
LG g1 sync "$CH" >/dev/null
ib11="$(LG g1 inbox "$CH" --thread git-thread 2>&1)"; ib11_rc=$?
[ "$ib11_rc" -eq 0 ] || { echo "FAIL [11]: inbox reprovou (rc $ib11_rc): $ib11"; exit 1; }
