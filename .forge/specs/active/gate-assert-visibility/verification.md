...
- **`LDG-0030` corrigido fora do escopo original**, durante o gate `design_reviewed`: `spec-transition.sh`/`validate-spec.mjs` não permitiam um `bugfix` scale≥2 pular `design-ready` mesmo com `quick_plan` justificado. Sem esse fix, este próprio change não teria conseguido avançar — não era opcional adiar. **A 1ª versão do fix estava errada** (removia `design-ready` da cadeia para `type: bugfix`, quebrando retrocompatibilidade de manifests legados já parados lá e proibindo a opção de design arquitetural que o próprio `design.md` command doc promete) — achado por 2 dos 3 reviewers do `code-evaluator` (ver abaixo) e corrigido com TDD real (teste vermelho escrito e confirmado falhando antes da correção, `tests/w21-pipeline-gate.sh` cenário `[1b]`): `design-ready` permanece na cadeia sempre; o pulo é uma rota lateral opcional para `type: bugfix` ou `quick_plan.skipped_phases` contendo `design`.
...
- **Três achados incidentais viraram itens de ledger** (`LDG-0031`, `LDG-0032`, mais `LDG-0033`/`0034`/`0035` do próprio `code-evaluator`) em vez de correções neste change — disciplina de escopo deliberada.
- **Dívida de processo resolvida nesta rodada**: os 32 commits desta sessão foram retroativamente movidos de `develop` direto para a branch `fix/tests/gate-assert-visibility` (PR #42), revisados por um `code-evaluator` adaptado (3 reviewers Opus/Sonnet independentes em paralelo — lógica/correção, qualidade/convenções, integridade do harness). **Todos os 3 reprovaram na 1ª rodada**: um `BLOCKER` convergente (plugin `plugin/forge/**` dessincronizado do template — `npm run build:plugin` não rodado após editar `template/.forge/commands/**`, recorreu 1x mais durante a própria correção) e 2 `HIGH` (fix do LDG-0030 sem cobertura de teste; quebra de retrocompatibilidade). Corrigidos nesta rodada (ver acima), suíte revalidada 100% verde no SHA final antes do merge.
