...
# Ele não toca a ponte de despacho `stdin` para `argv` nem o gerador `sync-adapters.mjs`, que são fronteira da Onda L1, não escreve em consumidor nenhum e NÃO faz censo das árvores de consumidor no disco. Censo de árvore externa deixaria esta suíte permanentemente vermelha em qualquer máquina que não seja a do especificador: o runner interno ganhou em LDG-0180 um terceiro estado ("não verificado", com código de saída próprio), mas ele é para desfecho que não é veredito do alvo — morte por sinal, dependência ausente, log não criado —, não é canal de skip por ausência de árvore de consumidor, e bloqueia o CI igual à reprovação. Um gate que dependesse das doze árvores continuaria derrubando a suíte inteira. O censo das doze árvores permanece como medição da especificação, e o que entra na suíte são as fixtures deste repositório.
#
# SEAM DE MUTAÇÃO.
